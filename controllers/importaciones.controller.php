...
if(isset($_GET['op'])){
    ...
    if ($_GET['op'] == 'generarExcelFonavi') {
        ...
        $filename = 'FormatoAportacionesFonavi.xlsx';
    ...
    if ($_GET['op'] == 'generarExcelPreparacionClases') {
        $numeroDoc = $_GET['numeroDoc'];
        $anioInicial = $_GET['anioInicial'];
        $anioFinal = $_GET['anioFinal'];
        ...
        $sheet->setCellValue('C13', 'D')->setCellValue('D13', 'M')->setCellValue('E13', 'A');
        $sheet->setCellValue('F13', 'D')->setCellValue('G13', 'M')->setCellValue('H13', 'A');
        // Ajustar las celdas para combinar correctamente
        $sheet->setCellValue('I11', 'REMUNERACION BRUTA')->mergeCells('I11:I12'); // Fusionar REMUNERACION BRUTA
        $sheet->setCellValue('K11', 'BONIFICACION ESPECIAL')->mergeCells('K11:K12'); // Fusionar BONIFICACION ESPECIAL
        $sheet->setCellValue('M11', 'DIRECCION / ENCARGATURA')->mergeCells('M11:M12'); // Fusionar DIRECCION / ENCARGATURA
        ...
        foreach ($resultados as $resultado) {
            ...
            // Colocar el valor de "totalRemuneracion" en la columna "REMUNERACION BRUTA" (columna I)
            $totalRemuneracion = $resultado['totalRemuneracion']; // Obtener el valor de totalRemuneracion
            $sheet->setCellValue("I$row", $totalRemuneracion);

            // Colocar el valor de "monto" en la columna "BONIFICACION ESPECIAL" (columna J)
            $bonificacionEspecial = $resultado['monto']; // Obtener el valor de monto
            $sheet->setCellValue("K$row", $bonificacionEspecial);
            ...
            $sheet->getStyle("G$row")->applyFromArray($rightAlign);
            $sheet->getStyle("H$row")->applyFromArray($rightAlign);
            $sheet->getStyle("I$row")->applyFromArray($rightAlign);  // Nueva columna "REMUNERACION BRUTA"
            $sheet->getStyle("K$row")->applyFromArray($rightAlign);  // Nueva columna "BONIFICACION ESPECIAL"
            $sheet->getStyle("M$row")->applyFromArray($rightAlign);  // Nueva columna

            // Aplicar bordes a las celdas de las instituciones, fechas y dirección
            $sheet->getStyle("A$row:B$row")->applyFromArray($borderStyle);
            $sheet->getStyle("C$row:H$row")->applyFromArray($borderStyle);
            $sheet->getStyle("I$row:J$row")->applyFromArray($borderStyle);  // Columnas "REMUNERACION BRUTA" y "BONIFICACION ESPECIAL"
            $sheet->getStyle("M$row:N$row")->applyFromArray($borderStyle);  // Nueva columna
        ...
        $sheet->getStyle('A1:K4')->applyFromArray($rightAlign + $boldStyle); // Títulos principales
        $sheet->getStyle('A6:K9')->applyFromArray($boldStyle); // Datos del trabajador
        $sheet->getStyle('A11:N20')->applyFromArray($borderStyle); // Solo la tabla tiene bordes
        $sheet->getStyle('C13:H13')->applyFromArray($rightAlign + $boldStyle); // Etiquetas de D, M, A
        ...
        $sheet->getColumnDimension('A')->setWidth(65); // Agrega un margen de 5 a la longitud máxima
         ...
         // Aplicamos bordes a la tabla completa
         $sheet->getStyle('A11:N' . $row)->applyFromArray($borderStyle);
        ...
        // Guardar el archivo
        $writer = new Xlsx($spreadsheet);
        $filename = 'FormatoPreparacionClases.xlsx';
